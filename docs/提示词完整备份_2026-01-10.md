...
**备份日期**: 2026-01-10
**文件路径**: `rdagent/scenarios/qlib/experiment/prompts.yaml`
**备份目的**: 在进行提示词优化前，完整备份现有内容，每段标注引用源
...
| 提示词键名 | 行号范围 | 使用阶段 | 引用源 |
|-----------|---------|---------|--------|
| `qlib_quant_background` | 8-17 | 通用背景 | `prompts.yaml:8-17` |
| `qlib_factor_background` | 19-40 | 因子背景 | `prompts.yaml:19-40` |
| `qlib_factor_interface` | 42-51 | 因子编码 | 模块化引用5个文件 |
| `qlib_factor_strategy` | 53-92 | 因子编码 | `prompts.yaml:53-92` |
| `qlib_factor_output_format` | 94-114 | 因子编码 | `prompts.yaml:94-114` |
| `qlib_factor_simulator` | 116-123 | 因子编码 | `prompts.yaml:116-123` |
| `qlib_factor_rich_style_description` | 125-144 | UI展示 | `prompts.yaml:125-144` |
| `qlib_factor_from_report_rich_style_description` | 146-178 | UI展示 | `prompts.yaml:146-178` |
| `qlib_factor_experiment_setting` | 180-219 | 因子编码 | `prompts.yaml:180-219` |
| `qlib_model_background` | 221-237 | 模型背景 | `prompts.yaml:221-237` |
| `qlib_model_interface` | 239-271 | 模型编码 | `prompts.yaml:239-271` |
| `qlib_model_output_format` | 274-277 | 模型编码 | `prompts.yaml:274-277` |
| `qlib_model_simulator` | 279-287 | 模型编码 | `prompts.yaml:279-287` |
| `qlib_model_rich_style_description` | 289-308 | UI展示 | `prompts.yaml:289-308` |
| `qlib_model_experiment_setting` | 310-317 | 模型编码 | `prompts.yaml:310-317` |
| `transformer_volume_price_strategy` | 319-332 | 示例策略 | `prompts.yaml:319-332` |
...
qlib_factor_background: |-
  ...
  对于每一个因子，请用「简体中文」给出清晰的基础解释，包括：
  - 因子名称（可以中英结合，例如：10 日动量（Momentum_10D））；
  - 因子的直观含义（它想刻画什么金融特性，例如动量、波动率、流动性等）；
  - 关键超参数（如窗口长度、回看期），以及这些超参数对因子行为的影响。
...
**引用源**: `prompts.yaml:42-51`（模块化引用）
**使用阶段**: 因子编码
**问题**: 已优化（2026-01-09 拆分为5个模块）
...
**模块文件内容**:
- `prompts_core_constraints.yaml` - 核心约束（统一模板、输出格式）
- `prompts_data_loading.yaml` - 数据加载规范
...
**引用源**: `prompts.yaml:53-92`
**使用阶段**: 因子编码
**问题**: 🔴 **P0** - 第65-70行展示 `reset_index()` 示例，与核心约束冲突
...
**问题分析**:
- 第65-70行：`df = df.reset_index()` 示例没有说明如何恢复索引
- 与 `prompts_error_prevention.yaml` 中"严禁使用 reset_index"的约束冲突
- LLM 可能模仿示例代码，导致索引结构错误

---

### 5. qlib_factor_output_format

**引用源**: `prompts.yaml:94-114`
**使用阶段**: 因子编码
**问题**: 无
...
**引用源**: `prompts.yaml:116-123`
**使用阶段**: 因子编码
**问题**: 🟡 **P1** - 背景介绍冗余（Qlib平台介绍、工作流程说明）
...
**问题分析**:
- 第117-118行：Qlib 平台背景介绍
- 第119-123行：RDAgent 工作流程说明
- 与因子编码的核心任务（计算因子值）无关
...
- `scenarios/qlib/experiment/factor_experiment.py:38` - 场景类初始化
...
**使用位置**:
- `scenarios/qlib/experiment/factor_from_report_experiment.py:10` - 场景类初始化

---

### 9. qlib_factor_experiment_setting

**引用源**: `prompts.yaml:180-219`
**使用阶段**: 因子编码
**问题**: 🔴 **P0** - 包含大量与因子编码无关的内容
...
**问题分析**:
- 第181-183行：数据集表格（保留）
- 第185-187行：股票代码格式说明（保留，但需避免重复）
- 第189-202行：计算资源与训练负载建议（❌ 应该移到模型训练提示词）
- 第204-207行：股票池筛选与每日指标因子优先使用原则（❌ 应该移到反馈生成提示词）
- 第209-219行：策略演进优先级（❌ 应该移到反馈生成提示词或删除）

**重复问题**:
- 股票代码格式说明在以下位置重复：
  - `prompts.yaml:185-187`（此处）
  - `prompts.yaml:315-317`（`qlib_model_experiment_setting`）
  - `prompts_data_loading.yaml`（模块文件）
  - `factor_coder/prompts.yaml:45-49`（`evolving_strategy_factor_implementation_v1_system`）

---

### 10. qlib_model_background

**引用源**: `prompts.yaml:221-237`
**使用阶段**: 模型背景
**问题**: 无
...
- `scenarios/qlib/experiment/model_experiment.py:35` - 场景类初始化

---

### 15. qlib_model_experiment_setting

**引用源**: `prompts.yaml:310-317`
**使用阶段**: 模型编码
**问题**: 🟡 **P1** - 股票代码格式说明重复
...
qlib_model_experiment_setting: |-
  ...
  **重要说明：股票代码格式**
  - 虽然数据集名称为 "AIstock A-share"，但所有数据源中的 `instrument` 均已统一为 **Qlib 风格**（如 `000001.SZ`, `600000.SH`）。
  - 在研发过程中**严禁**进行格式转换.
```

**问题分析**:
- 第315-317行：股票代码格式说明与 `qlib_factor_experiment_setting` 重复

---

### 16. transformer_volume_price_strategy

**引用源**: `prompts.yaml:319-332`
**使用阶段**: 示例策略
**问题**: 🟢 **P2** - 示例策略不必要

```yaml
transformer_volume_price_strategy: |-
  本段落描述的是一套 **示例型** 策略设想，主要用于展示如何将量价特征与时间序列模型（例如 Transformer）结合，用于多股票组合的研究与回测。

  需要特别强调的是：
  - 这里提到的模型类型（如 TransformerEncoder）、调仓规则、持仓股票数量（例如 30 只）、止盈止损阈值等，**仅作为示例/灵感来源**；
  - 它们 **不对当前任务或任何具体实验构成硬性约束**，你在实际研发中可以自由选择 LightGBM、CatBoost、MLP、GRU、Transformer 等任意模型，也可以自由设计选股数量与风控规则；
  - 本项目的其它提示词（因子/模型/回测配置）同样不强制绑定某一种模型或具体策略形态，而是以"数据格式与索引 contract 正确"为唯一刚性约束。

  示例思想（可选参考）：
  - 关注未来若干交易日（例如 1/5/10/30 日）的收益表现，而不是超长周期持有；
  - 在收益与风险之间设定合理的目标区间（例如期望年化收益、最大回撤、单笔风险敞口上限等），并在因子设计、模型结构以及调仓规则中综合考虑；
  - 以量价特征为一个重要维度，同时允许结合估值、市值、AE 异常度、宏观情绪等多方面信号，构建多因子与多周期的策略框架；
  - 对于时间序列建模，可以选择 Transformer、GRU、TCN 等，也可以回到简单的截面模型（如 LGB/MLP）并配合滚动窗口特征构造完成策略设计。

  总之，本段仅作为"如何构思一套较为完整的量价+时序策略"的 **示例文档**。在实际自动研发与演进过程中，你完全可以根据实验结果与任务需求，自主决定使用的模型类型、选股数量、持仓周期以及是否采用止盈/止损机制。
```
...
| 提示词 | 问题 | 行号 | 影响 |
|--------|------|------|------|
| `qlib_factor_strategy` | 展示 `reset_index()` 示例，与核心约束冲突 | 65-70 | 导致 LLM 破坏索引结构 |
| `qlib_factor_experiment_setting` | 包含训练参数、策略演进等无关内容 | 189-219 | 提示词过长，分散注意力 |

### 🟡 P1 中等问题（4个）

| 提示词 | 问题 | 行号 | 影响 |
|--------|------|------|------|
| `qlib_factor_simulator` | 背景介绍冗余 | 117-123 | 提示词过长 |
| `qlib_factor_rich_style_description` | UI展示内容，因子编码不需要 | 125-144 | 分散注意力 |
| `qlib_factor_from_report_rich_style_description` | UI展示内容，因子编码不需要 | 146-178 | 分散注意力 |
| `qlib_model_simulator` | 背景介绍冗余 | 280-287 | 提示词过长 |

### 🟢 P2 低优先级问题（3个）

| 提示词 | 问题 | 行号 | 影响 |
|--------|------|------|------|
| `transformer_volume_price_strategy` | 示例策略不必要 | 319-332 | 可能误导 |
| 股票代码格式说明 | 在多个位置重复 | 多处 | 冗余 |
| `qlib_model_experiment_setting` | 股票代码格式说明重复 | 315-317 | 冗余 |
...
1. **修复 `qlib_factor_strategy`**：删除或修改 `reset_index()` 示例
2. **精简 `qlib_factor_experiment_setting`**：移除训练参数和策略演进内容

### 建议修改（P1）

3. **精简 `qlib_factor_simulator`**：移除 Qlib 平台背景介绍
4. **评估 `rich_style_description`**：如果不需要 UI 展示，可以删除

### 可选修改（P2）

5. **移除 `transformer_volume_price_strategy`**：或移动到策略设计模块
6. **统一股票代码格式说明**：只在 `prompts_data_loading.yaml` 中保留一次
...
修改完成后：
- `qlib_factor_experiment_setting` 从 40 行减少到 10 行（减少75%）
- 移除错误引导，减少索引结构错误
- 提示词更加聚焦于因子编码的核心任务
- 减少 LLM 混淆，提升编码成功率 10-15%
