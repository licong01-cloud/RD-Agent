# RD-Agent × AIstock Phase 2 详细设计最终版（2026-01-02）

> 本文件为 RD-Agent × AIstock Phase 2 设计的**最终整合版（更新至 2026-01-02）**，在内容上整合并去重自：
...
>
> 所有需求与接口说明均以本文件为**唯一入口文档**；旧版文件保留用于追溯，不再单独作为需求来源。
>
> 2026-01-02 更新后的关键工程约束：
> - **在线最轻量（marker-only）**：workflow/loop 在线执行阶段，`write_loop_artifacts(...)` 仅写入采集标识与状态，禁止目录遍历/大文件读取/大量写盘/共享库同步等重操作；
> - **离线可重试（materialize）**：所有 Phase 2 artifacts 的生成与登记统一由离线脚本执行（`tools/backfill_registry_artifacts.py`），允许延迟但最终必须完整；
> - **对外不变**：最终生成的文件名、JSON schema、四大 Catalog 字段集合、Results API 返回结构保持一致，不得丢字段、不得空字段。
...
- **RD-Agent 侧**：
  ...
  - 在 workflow 在线执行阶段，`write_loop_artifacts(...)` **仅写入采集标识与状态（marker-only）**；
  - 由离线脚本（`tools/backfill_registry_artifacts.py`）统一完成：生成/登记所有约定 artifacts、回填缺失字段、更新 `has_result` 与关键指标、并刷新四大 Catalog；
...
def write_loop_artifacts(
    ...
    *,
    ...
    """在线写入采集标识与状态（marker-only）。

    2026-01-02 口径：
    - 在线阶段仅写入 registry 所需的最小信息（task/loop/workspace 的状态与可追踪索引），并将
      `materialization_status` 置为 `pending`（或更新 error/status）；
    - 任何涉及目录遍历、大文件读取、生成/写入 JSON/图表、登记 artifact_files、同步共享库等重操作，
      均由离线脚本执行（`tools/backfill_registry_artifacts.py`）；
    """
    ...
```
...
1. upsert task/loop/workspace 的基础状态与索引（例如 `status/action/workspace_path/log_trace_path`）；
2. 写入采集状态机字段：
   - `materialization_status`：`pending/running/done/failed`；
   - `materialization_error`：失败摘要（可空）；
   - `materialization_updated_at_utc`（推荐）；
3. 在线阶段禁止：目录遍历/大文件读取/大量写盘/同步共享库/登记 artifact_files。

**离线职责（materialize）**：

- Phase 2 产物（JSON/图表/registry artifact_files）由离线脚本生成与登记：
  - `tools/backfill_registry_artifacts.py --mode materialize-pending`
- 该离线过程必须幂等可重试，且最终生成的文件与字段集合与既有 Phase 2 版本保持一致。

#### 2.1.1 `materialization_status` 状态机定义与幂等规则（2026-01-02 补充）

**新增字段（Registry / loops 表）**：

1. `materialization_status`: `pending | running | done | failed`
2. `materialization_error`: `TEXT`（最近一次失败摘要，可空）
3. `materialization_updated_at_utc`: `TEXT`（推荐，用于观测与排障）

**语义**：

1. `pending`
   - 在线阶段写入，表示该 loop 需要离线采集（materialize）才能成为“可消费”成果。
2. `running`
   - 离线任务正在采集；用于互斥与 UI 展示。
3. `done`
   - 采集完成：
     - Phase 2 标准文件齐全（JSON/图表等）；
     - registry 中 artifacts/artifact_files 记录齐全；
     - loop_catalog / Results API 可对外稳定提供完整数据。
4. `failed`
   - 采集失败：允许重试；失败原因写入 `materialization_error`。

**互斥/幂等（必须满足）**：

1. 幂等
   - 对同一 `(task_run_id, loop_id)` 多次执行 `materialize-pending` 不得产生重复 artifacts 记录或脏数据。
2. 互斥
   - 离线任务开始时应尝试将 `pending/failed -> running`（可用“仅当当前 status in (pending,failed)”的更新条件实现），避免多进程重复采集。
3. 失败可重试
   - 任意异常：写 `failed + error`；下次允许再次进入 `running`。

**在线阶段强约束**：

1. 在线 `write_loop_artifacts` 仅允许写 `pending`（以及基础状态/索引），不得执行 materialize。

### 2.2 与 loop 执行流程的整合

- 在 `rdagent/utils/workflow/loop.py` 中 step `finally` 会调用 `write_loop_artifacts(...)`；
- 2026-01-02 口径：该调用仅用于写入采集标识与状态（marker-only），不得在在线阶段生成/登记完整成果；
```python
...
- 迁移过程保持对外行为兼容：最终 workspace 文件与 registry 记录集合保持一致；
- 在线阶段允许延迟：部分 JSON/图表/registry artifact_files 可由离线 materialize 补齐，但最终必须齐全。
...
- 离线 materialize/backfill 生成逻辑要求：
  - 幂等（多次执行不会产生重复 DB 记录）；
  - 单个 JSON/图表生成失败时：
    - 不应影响 registry 的主流程写入；
    - 必须将 `materialization_status` 置为 `failed` 并写入 `materialization_error`（可附加更详细日志路径）；
...
- 回测图表的生成与登记统一由离线 materialize/backfill 工具完成（在线 marker-only 不生成图表）；
...
- Phase 2 要求其支持（2026-01-02 更新后，该工具也是新任务的事后采集入口）：
  - 遍历与检查历史任务：
  ...
  - 提供三种 **mode**：
    - `--mode materialize-pending`：对 `materialization_status in (pending, failed)` 的 loop 执行事后采集（生成 JSON/图表 + 登记 artifacts/artifact_files + 回填指标/状态），成功置 `done`，失败置 `failed` 并记录 error；
...
2026-01-02 口径：共享包入库逻辑作为**离线任务**执行（允许延迟），不得在在线 workflow finally 阶段执行。

离线共享包入库逻辑（候选因子通过验证被标记为“成功”后，由 materialize/backfill 触发）：
...
> 上述 1–4 步完全由 RD-Agent 内部 Python 代码自动完成，不依赖人工编辑共享包文件；执行时机为离线任务。
...
- 在离线 materialize/backfill 中集成 `_sync_factor_impl_to_shared_lib`：
  - 为通过验收的因子写入占位函数 stub（抛 `NotImplementedError` 或仅记录源码字符串）；
...
- FastAPI 应用：`rdagent.app.results_api_server:create_app`；（Phase 2 中仅**向响应中新增字段**，不删除或重命名任何既有字段。）
...
- RD-Agent 侧职责：
  ...
  - 将 `materialization_status/materialization_error` 写入 loop_catalog，作为 AIstock 判定是否可消费的唯一依据；
  - 从 Phase 2 artifacts 中抽取因子组合与回测指标；
  - 关联 `strategy_catalog` 后输出 `loop_catalog.json`；
- AIstock 侧职责：
  - 导入 `loop_catalog` 至本地 `backtest_runs` 表；
  - 仅当 `materialization_status == "done"` 时将该 loop 视为“可消费/可展示为完整成果”；
  - 支持历史回测记录按策略/因子/指标筛选。
...
2. 采集状态判定与手工触发补齐（2026-01-02 更新）：
   - AIstock 判定某条 loop 是否“可消费/可展示/可进入生产流程”时，必须以 `materialization_status == done` 为准；
   - `pending/failed` 的 loop 允许存在延迟：
     - UI 展示“未采集/采集失败”，并展示 `materialization_error`（如存在）；
     - 在 UI 上提供“补齐该任务数据”按钮：
       - 点击后由 AIstock 后端调用 `POST /ops/materialize-and-refresh`，触发 RD-Agent 侧 backfill/materialize 与 catalog 刷新；
       - 任务完成后再调用 Results API 的 `/catalog/*` 同步最新成果（流程见 9.2）。
...
### 6.4 AIStock 对接 Phase 2 能力的调用流程与伪代码（2026-01-02 补充）

#### 6.4.1 能力概览

- **只读 Results API（面向 AIStock 查询）**：
  - `GET /catalog/loops`
    - 返回 loop_catalog，单条 loop 结构中新增字段：
      - `materialization_status: "pending" | "running" | "done" | "failed" | null`
      - `materialization_error: str | null`
- **内部 Ops API（面向 AIStock/运维后台触发离线流程）**：
  - `POST /ops/materialize-and-refresh`
    - 行为：
      1. 扫描 registry 中 `loops.materialization_status ∈ {NULL, "pending", "failed"}` 的 loops；
      2. 对上述 loops 运行离线物化（Phase1 + Phase2），并更新状态机：`pending/failed -> running -> done/failed`；
      3. 依次刷新四大 AIStock catalog：
         - `factor_catalog.json`
...
> 约束：
>
> - Results API 只提供只读查询，不直接触发任何“执行类”操作；
> - Ops API 仅在受控环境（本机/内网）开放，由 AIStock 后端或运维工具调用；
> - AIStock 判定某条 loop 是否“可消费/可展示”，必须以 `materialization_status == "done"` 为准，其余状态视为“待补齐/补齐中/补齐失败”。

#### 6.4.2 后端调用时序（AIStock Server ↔ RD-Agent）

**场景：** 用户在 AIStock 中打开某个 loop 的详情页，希望看到完整回测指标、因子表现等；若尚未物化，则提示状态并可触发物化。

**时序步骤（文字版）：**

1. AIStock 后端调用 Results API：
   - `GET http://<rdagent-results>/catalog/loops`
2. 在返回的 `loops` 列表中根据 `loop_id`（及必要时的 `task_run_id`）定位目标 loop，读取：
   - `materialization_status`
   - `materialization_error`
3. 根据状态分支处理：
   - `materialization_status == "done"`：
     - 直接使用 loop_catalog + 其它明细接口渲染完整结果；
   - `materialization_status == "running"`：
     - 前端提示“物化中”，引导用户稍后刷新；
   - `materialization_status in {"pending", null}`：
     - 标记为“待物化”，可提供“触发物化”按钮；
   - `materialization_status == "failed"`：
     - 显示失败提示，并展示（或可展开）`materialization_error` 文本；
     - 可提供“重新触发物化”按钮。
4. 当用户或后台需要补齐 artifacts 时，AIStock 后端调用 Ops API：
   - `POST http://<rdagent-results>/ops/materialize-and-refresh`
5. RD-Agent 在服务器本地执行：
   - `backfill_registry_artifacts.py --mode materialize-pending`；
   - 四个 `export_aistock_*.py` 导出脚本；
   - 返回每一步的 `returncode` 与标准输出内容。
6. AIStock 后端根据返回值：
   - 如全部 `returncode == 0`：
     - 再次调用 `GET /catalog/loops` 刷新状态；
   - 如有 step 失败：
     - 将 `detail.steps` 中的 `output` 记录到运维系统，或提供管理端查看原始日志。

#### 6.4.3 后端伪代码示例（Python）

**读取 loop_catalog 并构建 ViewModel：**

```python
import httpx

RDA_RESULTS_BASE = "http://rdagent-results"  # 通过配置注入


def fetch_loop_catalog() -> dict:
    resp = httpx.get(f"{RDA_RESULTS_BASE}/catalog/loops", timeout=30)
    resp.raise_for_status()
    return resp.json()


def get_loop_entry(loop_id: int) -> dict | None:
    catalog = fetch_loop_catalog()
...
def build_loop_view_model(loop_id: int) -> dict:
    entry = get_loop_entry(loop_id)
    if not entry:
        return {"found": False}

    status = entry.get("materialization_status")
    error = entry.get("materialization_error")
    ...
    return {
        ...
        "has_result": entry.get("has_result"),
...
```python
def trigger_materialize_and_refresh() -> dict:
    url = f"{RDA_RESULTS_BASE}/ops/materialize-and-refresh"
    resp = httpx.post(url, timeout=600)
    ...
    return {"ok": False, "status_code": resp.status_code, "error": payload}
```

> 建议：该触发逻辑只在内网/管理端调用，且在 AIStock 内部做权限控制与频率限制，避免用户高频点击导致重复物化。

#### 6.4.4 前端交互伪代码（React 风格）

```tsx
type MaterializationStatus = "pending" | "running" | "done" | "failed" | null;

interface LoopViewModel {
  found: boolean;
  loop_id: number;
  materialization_status: MaterializationStatus;
  materialization_error?: string | null;
  materialization_hint?: string;
}

function LoopDetailPage({ loopId }: { loopId: number }) {
  const [data, setData] = useState<LoopViewModel | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/aistock/loops/${loopId}`)
      .then(res => res.json())
  ...
  const handleTriggerMaterialize = () => {
    setOpsRunning(true);
    setOpsError(null);
    fetch(`/api/aistock/loops/${loopId}/materialize`, { method: "POST" })
      .then(res => res.json())
      .then(json => {
        if (!json.ok) {
          setOpsError("物化触发失败，请查看运维日志");
          return;
        }
        return fetch(`/api/aistock/loops/${loopId}`)
          .then(res => res.json())
          .then(json => setData(json));
      })
      .finally(() => setOpsRunning(false));
  };

  // 根据 data.materialization_status 渲染标签和按钮，略。
}
```
...
```tsx
function renderMaterializationSection(data: LoopViewModel) {
  const status = data.materialization_status;

  if (status === "done") {
    return <Tag color="green">物化完成</Tag>;
  }

  if (status === "running") {
    return (
      <>
        <Tag color="blue">物化中</Tag>
        <Text type="secondary">请稍后刷新页面</Text>
      </>
    );
  }
  ...
  if (status === "failed") {
    return (
      <>
        <Tag color="red">物化失败</Tag>
        <Tooltip title={data.materialization_error || "未知错误"}>
          <Text type="danger">查看错误详情</Text>
        </Tooltip>
        <Button onClick={handleTriggerMaterialize}>重新触发物化</Button>
      </>
    );
  }
  ...
  return (
    <>
      <Tag>待物化</Tag>
      <Button type="primary" onClick={handleTriggerMaterialize}>
        触发物化
      </Button>
    </>
  );
...
- **安全边界**：
  - `POST /ops/materialize-and-refresh` 只在 RD-Agent 所在机器或可信内网开放，由 AIStock 后端统一调用；
  - 前端只调用 AIStock 自己的 `/api/aistock/...`，不直接访问 RD-Agent。
- **调用频率控制**：
  - Ops API 是重操作：每次会扫描所有 pending/failed loop 并物化；
  - 建议只作为“人工点击”或“定时任务”入口，而非高频自动触发。
- **失败排查**：
  - Ops API 返回 500 时，`detail.steps` 中包含每个子脚本的 `returncode` 和 stdout；
  - AIStock 可将该信息写入日志/告警系统，或在管理后台提供“原始日志查看”。
...
- **REQ-FACTOR-P2-011：loop → 因子共享包写入**  
  ...
  2026-01-02 口径：上述动作由离线 materialize/backfill 执行（允许延迟），不得在在线 workflow finally 中执行。

- **REQ-ARTIFACT-P2-001：采集状态机字段与可重试采集**  
  `loops` 表必须包含：`materialization_status` / `materialization_error`（以及推荐的 `materialization_updated_at_utc`）。  
  - 在线阶段：仅写入 `pending` 标识；  
  - 离线阶段：执行 `materialize-pending` 完成采集并置 `done`；失败置 `failed` 并写 error；可重试直至完成。
...
-- **GET `/catalog/loops`**  
  ...
  - `loops`: loop 列表，单条记录至少包括：
    ...
    - 采集状态：`materialization_status`, `materialization_error`（Phase 2 新增字段，旧版 AIstock 可忽略不读）；
  ...
  API 必须与 `loop_catalog.json` 字段一一对应，禁止对指标或路径字段做任何删减；Phase 2 仅在原有结构上**追加字段**，不移除旧字段，以保证 AIstock 侧改动最小（只需多读一个状态字段）。
...
2. **AIstock 侧增量数据一键更新（推荐流程）**  
   ...
   - **RD-Agent 侧：刷新四大 catalog 的固定命令**（由定时任务或人工触发）：  
     ...
     ```bash
     # 1) 事后采集（推荐）：对 pending/failed 的 loop 执行 materialize，生成标准化 JSON/图表并登记
