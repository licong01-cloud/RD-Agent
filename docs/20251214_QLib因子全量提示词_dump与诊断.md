# 2025-12-14：Qlib 因子全量提示词（prompts.yaml）Dump 与静态诊断

> 目的：
> - 将当前 `rdagent/scenarios/qlib/experiment/prompts.yaml` 的全量提示词一次性打印出来，便于审阅/备案。
> - 对提示词做静态诊断：是否存在重复描述、潜在矛盾、或容易导致 LLM 误解的表述。
>
> 说明：本文仅做“打印+诊断”。提示词的实际生效与改动以 `prompts.yaml` 为准。

---

## 1. 全量提示词原文（来自 `rdagent/scenarios/qlib/experiment/prompts.yaml`）
...
qlib_factor_interface: |-
  ...
  - 因子脚本应严格采用如下整体骨架，其中 **仅允许在标记的“FACTOR COMPUTATION AREA”内编写/修改代码**.
    ...
    - 可以综合利用 Qlib 提供的 Alpha158 特征、预计算的 AE 因子与 daily_basic 因子（在上游统一拼接后作为模型输入特征）来构造更复杂、更多样化的因子与策略；
  ...
  - **可选静态字段（daily_basic / 资金流原始字段）**：当前工作目录下可能存在 `static_factors.parquet`（由上游数据准备阶段复制到因子执行目录），其索引同样为 MultiIndex(`datetime`, `instrument`)，包含 daily_basic / moneyflow 等原始字段（通常以 `db_` / `mf_` 等前缀命名）。
    ...
    - 如果你的因子定义确实依赖这些字段（例如流通市值、流通股本、换手率、资金净流入等），你 **必须** 显式读取并 join：
      ```python
  ...
  【严禁硬编码替代变量（必须严格遵守）】
  - 如果你的因子在数学定义上需要某些字段（例如流通市值/流通股本/换手率/市值/估值/资金流字段），但这些字段在 `daily_pv.h5` 与 `static_factors.parquet` 中不存在：
    - **必须** `raise ValueError` 明确指出缺失字段；
    - 并在下一轮实验中 **改写因子定义**，仅使用当前可获得的字段构造可计算的替代信号。
  ...
  - 因子结果必须保持索引为 MultiIndex(`datetime`, `instrument`)，`index.names == ["datetime", "instrument"]`，并通过：
    ...
    result_df.to_hdf("result.h5", key="data", mode="w")
    ```
    写入同一目录下的 `result.h5` 文件，列为一个或多个 `float64` 类型的因子列。
...
qlib_factor_experiment_setting: |-
  ...
  - **链路稳定优先**：
    - 在加大训练负载前，先用较小循环（如 `--loop-n 1`）确认整条因子与 Qlib 训练链路完全打通：
      - 使用存在的股票池（例如 `instruments: all` 或已经在 snapshot 中提供了 `aistock.txt`）。
...
qlib_model_experiment_setting: |-
  | Dataset 📊 | Model 🤖    | Factors 🌟       | Data Split  🧮                                   |
  |---------|----------|---------------|-------------------------------------------------|
  | AIstock A-share (non-ST, non-delisted)  | 例如 GRU_TimeSeries_Model（也可替换为 LGB/MLP/Transformer 等其它模型） | 20 factors (Alpha158)  | Train: 2016-01-01 to 2018-12-31 <br> Valid: 2019-01-01 to 2020-12-31 <br> Test &nbsp;: 2021-01-01 to 2025-12-31 |
...
### 2.1 易误解点：`factor.py` 与 “Alpha158/预计算因子作为输入” 的关系
- **现状**：`qlib_factor_interface` 的“FACTOR COMPUTATION AREA”说明中出现“可以综合利用 Qlib 提供的 Alpha158 特征、预计算 AE/daily_basic 因子…”。
- **风险**：在当前因子实现 contract 下，`factor.py` 运行时的**硬约束输入**只有：
  - `daily_pv.h5`（价量）
  - 可选的 `static_factors.parquet`（需要显式 `read_parquet+join`）
  - 并不天然包含 Alpha158（Alpha158 往往是 Qlib handler 端在训练/回测时生成的特征，而不是 `daily_pv.h5` 直接带出来的列）。
- **建议**：将该句改写为“预计算因子会在 Qlib 侧作为特征拼接（模型输入），但单个 `factor.py` 的运行时只保证 `daily_pv.h5` +（可选 join）`static_factors.parquet`”。

### 2.2 重复描述：数据重命名规则出现两次
- 同一段落中同时给出了：
  - 模板骨架里的 `rename_map`；
  - 后面“数据加载与字段名规范”里又给了一遍相同的 `rename_map`。
- **影响**：对 LLM 来说属于轻微冗余，但不构成矛盾。
- **建议**：可保留（用于强调刚性约束），也可在未来做一次性收敛。

### 2.3 反例字段名的出现（`mf_net_inflow_1d`）
- `prompts.yaml` 中提到 `mf_net_inflow_1d` 是为了作为“禁止编造字段”的反例。
- **风险**：如果 prompt 过长或模型截断，反例可能被模型误当“可用字段”。
- **建议**：保持“反例”但在文案上加更强的否定语气（当前已经是“严禁…例如”）；同时确保 schema 文件能被注入（本次已补齐 debug 目录 schema，且支持 .csv/.json 描述）。

### 2.4 `instruments: all` 与 `aistock.txt` 的表述
- `qlib_factor_experiment_setting` 里提到“`instruments: all` 或 snapshot 中提供了 `aistock.txt`”。
- **风险**：在不同 snapshot 导出策略下，可能只有 `all.txt`。
- **建议**：后续可将该句收敛为“使用 snapshot 中实际存在的 instruments 文件（如 `all.txt`）”。
