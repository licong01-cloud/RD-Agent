...
> **【新增说明 / 2025-12-22】Windows 本地开发默认导入目录**
>
> - 为方便本地开发与 AIstock 后端统一路径解析，AIstock 侧新增环境变量：
>   - `PROMPT_PACK_INBOX_DIR="F:\\Dev\\AIstock\\prompt_packs_inbox"`
> - 当调用 `validate-import-dir` / `import-from-dir` 时：
>   - 若 `dir` 传入 **绝对路径**（如 `F:\\Dev\\AIstock\\prompt_packs_inbox\\<pack_id>`），则直接使用该路径
>   - 若 `dir` 传入 **相对路径或 pack_id**（如 `<pack_id>`），则 AIstock 后端会按 `PROMPT_PACK_INBOX_DIR/<pack_id>` 自动拼接解析
> - 该逻辑是 AIstock 侧路径解析增强，RD-Agent 侧无需改动；此处仅为跨团队对齐。

> **【新增说明 / 2025-12-22】RD-Agent 在 WSL 环境访问 Windows 路径（/mnt 映射）**
>
> - 若 RD-Agent 在 WSL2 的 conda 环境运行，默认可通过 `/mnt/<drive>/...` 访问 Windows 文件系统
> - 以本项目使用的 F 盘为例：
>   - Windows：`F:\\Dev\\AIstock\\prompt_packs_inbox\\`
>   - WSL：`/mnt/f/Dev/AIstock/prompt_packs_inbox/`
> - 因此：当 RD-Agent 侧需要读取 AIstock 导出的共享文件（例如 Phase 1 的 cache 包）时，应优先使用 WSL 路径（如 `/mnt/f/...`），不要直接使用 `F:\\...`
> - 注意：`/mnt/*` 属于跨文件系统访问，频繁小文件读写可能较慢；若后续读写频率很高，可考虑将 cache 落在 WSL 文件系统目录（例如 `~/prompt_cache/`），再由同步脚本从 Windows 侧更新。
...
> **【新增说明 / 2025-12-22】dir 参数可传 pack_id（相对路径）**
>
> - 当配置了 `PROMPT_PACK_INBOX_DIR` 时，上述请求体可以简化为：`{ "dir": "<pack_id>" }`
> - 后端会将其解析为：`PROMPT_PACK_INBOX_DIR/<pack_id>`
> - 若目录不存在，错误返回会包含 `resolved_dir` 便于排障。
...
> **【新增说明 / 2025-12-22】WSL 读取 cache 路径示例**
>
> - 若 cache 文件落在 Windows 盘（例如 `F:\\Dev\\AIstock\\prompt_cache\\{active_pack_id}.json`），则 RD-Agent 在 WSL 侧应配置为：
>   - `RD_AGENT_PROMPT_CACHE_PATH=/mnt/f/Dev/AIstock/prompt_cache/{active_pack_id}.json`
