...
class QlibModelRunner(CachedRunner[QlibModelExperiment]):
    ...
    def develop(self, exp: QlibModelExperiment) -> QlibModelExperiment:
        ...
        # After a successful run, export model artifacts into session pickle by injecting bytes into file_dict.
        # This is required by AIstock strict Task-only sync: it reads weights ONLY from session.sub_workspace_list.file_dict.
        try:
            ws_path = getattr(getattr(exp, "experiment_workspace", None), "workspace_path", None)
            if isinstance(ws_path, Path) and exp.sub_workspace_list and exp.sub_workspace_list[0] is not None:
                sub_ws = exp.sub_workspace_list[0]
                injected = []
                for k in ["model.pkl", "params.pkl"]:
                    ...
                    if p.exists() and p.is_file():
                        try:
                            sub_ws.file_dict[k] = p.read_bytes()
                            injected.append(k)
                        except Exception as e:
                            logger.warning(f"[ModelArtifacts] Failed to read {p}: {e}")
                if injected:
                    logger.info(f"[ModelArtifacts] Injected {injected} into session file_dict (task={exp.sub_tasks[0].name})")
                else:
                    logger.warning(
                        f"[ModelArtifacts] No model weight artifacts found in workspace after success: {ws_path}. "
                        "Expected model.pkl or params.pkl generated by read_exp_res.py"
                    )
        except Exception as e:
            logger.warning(f"[ModelArtifacts] Unexpected error when injecting model artifacts into file_dict: {e}")
